...
$admin_email[] = 'ahulitochka@mail.ru';
...
foreach ( $_POST as $key => $value ) {
	if ( $value != ""  && $key != "admin_email" && $key != "form_subject" ) {
		...
		$message .= "
		" . ( ($c = !$c) ? '<tr>':'<tr>' ) . "
		<td style='padding: 10px; width: auto; background: rgb(237 245 247);'><b>$key:</b></td>
		<td style='padding: 10px;width: 100%; background: rgb(237 245 247);'>$value</td>
		</tr>
		";
	}
}
...
// От кого
$mail->setFrom('adm@' . $_SERVER['HTTP_HOST'], 'NVA-admin');
